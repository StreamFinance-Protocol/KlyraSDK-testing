...
import {
  ...
  accountFromAny,
  type Block,
  type QueryClient as StargateQueryClient,
  type TxExtension,
...
import { type TendermintClient } from "../lib/tendermint";
...
export default class NodeGet {
  ...
  async getWithdrawalAndTransferGatingStatus(
    ...
    const data = await this.sendQuery(
      "/klyraprotocol.subaccounts.Query/GetWithdrawalAndTransfersBlockedInfo",
      requestData
    );
  ...
  async getWithdrawalCapacityByDenom(
    ...
    const data = await this.sendQuery(
      "/klyraprotocol.ratelimit.Query/CapacityByDenom",
      requestData
    );
