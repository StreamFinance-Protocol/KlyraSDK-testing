...
import { type Block, TimeoutError, type IndexedTx } from "@cosmjs/stargate";
import {
  Method,
  type Tendermint37Client,
...
import { BroadcastErrorObject } from "./errors";
...
export class TendermintClient {
  ...
  async broadcastTransactionSync(
    ...
    const result: BroadcastTxSyncResponse =
      await this.baseClient.broadcastTxSync({ tx });
    if (result.code !== 0) {
   ...
   */
  async queryHash(hash: Uint8Array, time = 0): Promise<IndexedTx> {
    ...
    if (time >= this.broadcastOptions.timeoutMs) {
      throw new TimeoutError(
        `Transaction with hash [${hash}] was submitted but was not yet found on the chain. You might want to check later. Query timed out after ${
          this.broadcastOptions.timeoutMs / 1000
        } seconds.`,
        transactionId
      );
    }

    await sleep(this.broadcastOptions.pollIntervalMs);
